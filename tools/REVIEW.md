...
- 页面结构（参照 EasyNotes 官网与 11 个同类 app 官网调研）：产品页 = 首屏（右侧 4:5 浅底圆角面板：有预览视频的放缩小后完整的视频，否则放截图中部界面裁切、不带宣传大字）→ 功能图文（浅底圆角卡片：商店截图中部的手机界面按 4:5 裁切、不带宣传大字 + 标题 + 两三句，参照 EasyNotes）→ 截图画廊 → 商店原文细节 → 当前版本更新内容 → FAQ → 指南与免费工具 → 其他 app；语言切换只在页头（参照 GoFasting 的「EN ⌄」菜单），页面底部不再列语言；另有 `/blog/`（英文指南）与 `/tools/`。
